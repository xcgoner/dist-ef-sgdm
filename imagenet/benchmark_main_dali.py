...
class HybridTrainPipe(Pipeline):
    def __init__(self, rec_files, idx_files, batch_size, num_threads, device_id, data_dir, crop, dali_cpu=False):
        super(HybridTrainPipe, self).__init__(batch_size, num_threads, device_id, seed=12 + device_id)
        self.input = ops.MXNetReader(path = rec_files, index_path = idx_files, shard_id=args.rank, num_shards=args.world_size, random_shuffle=True)
...
class HybridValPipe(Pipeline):
    def __init__(self, rec_files, idx_files, batch_size, num_threads, device_id, data_dir, crop, size):
        super(HybridValPipe, self).__init__(batch_size, num_threads, device_id, seed=12 + device_id)
        self.input = ops.MXNetReader(path = rec_files, index_path = idx_files, shard_id=args.rank, num_shards=args.world_size, random_shuffle=False)
...
def get_parser():
    ...
    parser.add_argument('-vb', '--val-batch-size', default=25, type=int,
                        metavar='VN', help='val mini-batch size (default: 256)')
...
def main():
    ...
    train_loader,val_loader = get_loaders(traindir, valdir, split_data = not args.test_evaluate, seed = args.seed)
...
def validate(val_loader, model, criterion, epoch, start_time, log_writer):
    ...
    logging.info('val-acc: {}  {}'.format(top1.avg, top5.avg))
    return top1.avg
