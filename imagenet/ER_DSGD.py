...
class SGD_distribute(Optimizer):
    ...
    def __init__(self, params, args, log_writer, **kwargs):
        ...
        if self.compression_buffer:
            ...
            # p2p broadcast group
            self.inter_node_group_list = dict()
        ...
        for group in self.param_groups:
            ...
            for i, dev_grads in enumerate(dev_grads_buckets):
                ...
                if self.all_reduce:
                    ...
                    # hash the block to a worker
                    root_i = i % self.nodes
                    root_rank = self.inter_node_group[root_i]
                    ...
                    if self.nodes > 1:
                        ...
                        # all reduce gradient to the server node
                        if dist.get_rank() == root_rank:
                            d_p_new_list = []
                            d_p_new_scale_list = []
                            for j in range(len(self.inter_node_group)):
                                if j == root_i:
                                    continue
                                d_p_temp = d_p_new.clone()
                                d_p_scale_temp = d_p_new_scale.clone()
                                dist.broadcast(d_p_scale_temp, self.inter_node_group[j], group = self.inter_node_group_list[j][root_i])
                                dist.broadcast(d_p_temp, self.inter_node_group[j], group = self.inter_node_group_list[j][root_i])
                                d_p_new_list.append(d_p_temp)
                                d_p_new_scale_list.append(d_p_scale_temp)
                        else:
                            dist.broadcast(d_p_new_scale, dist.get_rank(), group = self.inter_node_group_list[self.nodes_rank][root_i])
                            dist.broadcast(d_p_new, dist.get_rank(), group = self.inter_node_group_list[self.nodes_rank][root_i])
                            dist.barrier(group = self.all_inter_node_group)

                        if dist.get_rank() == root_rank:
                            if self.compression_buffer:
                                # uncompress and sum up
                        ...
                        # broadcast
                        dist.broadcast(d_p_new, root_rank, group = self.all_inter_node_group)
                        if self.compression_buffer:
                            dist.broadcast(d_p_new_scale, root_rank, group = self.all_inter_node_group)
