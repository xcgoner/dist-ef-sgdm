...
@register
class DistEFSGD(Optimizer):
    def __init__(self, momentum=0.0, **kwargs):
        super(DistEFSGD, self).__init__(**kwargs)
        self.momentum = momentum

    def create_state(self, index, weight):
        wd_momentum = None
        if self.momentum != 0.0:
            wd_momentum = zeros(weight.shape, weight.context, dtype=weight.dtype, stype=weight.stype)
        return wd_momentum
    ...
    def update(self, index, weight, grad, state):
        ...
        # grad is tilde_delta, please refer to dist_ef_compr_sgd in src/kvstore/comm.h
        # rescale_grad is useless here, see comments in dist_ef_compr_sgd
        if self.momentum != 0.0:
            wd_mom = state
            wd_mom[:] *= self.momentum
            wd_mom[:] += wd * weight
            weight[:] -= lr * (self.momentum * wd_mom + wd * weight + grad)
        else:
...
class SignSGD(Optimizer):
    def __init__(self, **kwargs):
        super(SignSGD, self).__init__(**kwargs)
        # SignSGD doesn't have momentum argument, export MXNET_MOMENTUM=0.9
